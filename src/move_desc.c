...
describe_move(struct move_desc *desc, const struct position *pos, move m)
{
	...
	if (desc->move == 0 || mfrom(desc->move) != mfrom(m))
	...
	desc->value += popcnt(desc->dst_sq.attacks & ~desc->src_sq.attacks);
	desc->value -= popcnt(desc->src_sq.attacks & ~desc->dst_sq.attacks);
}
