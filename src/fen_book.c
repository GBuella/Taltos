...
static void
parse_raw(char *raw, struct fen_book *book, jmp_buf jb)
{
    book->count = 0;
    raw += strspn(raw, "\n\r");
    while (*raw != '\0') {
...
*fen_book_open(const char *path)
{
    ...
    parse_raw(book->raw, &book->fen_book, jb);
...
fen_book_parse(const char * const * volatile raw)
{
    ...
    struct book *book = xcalloc(sizeof *book);
    jmp_buf jb;
    ...
    do {
        if (book->fen_book.count == ARRAY_LENGTH(book->fen_book.entries))
            longjmp(jb, 1);
