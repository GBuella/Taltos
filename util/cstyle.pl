...
line: while (<$filehandle>) {
	...
	if ($attribute_must_follow) {
		$attribute_must_follow = 0;
		if (/^\s*{/) {
			err("return type of function not on separate line");
		}
		if (!/^\s*attribute\(.*\);$/) {
			err("function declaration expected");
		}
	}
...
#		}
	...
	if (/\b($typename|void)\s\s*\*[,\)]/o ||
	    /\b($typename|void)\s\s*\*$/o) {
		err("extra space between type name and *");
	}
	if (/^\s+#/) {
	...
	if (/}\s*else\W/) {
		err_prefix($prev,
		    "else and right brace should not be on same line");
	}
	$prev = $line;
}
