...
static void
gen_simple_table(const int dirs_v[8], const int dirs_h[8])
{
...
static void
add_rays(int r, int f, int r_dir, int f_dir)
{
	int src_i = ind(r, f);
	uint64_t ray = EMPTY;
	int dst_r = r + r_dir;
...
static void
gen_magics(const int dirs[4], const uint64_t edges[4])
{
...
static void
gen_bishop_patterns(void)
{
	for (int i = 0; i < 64; ++i) {
		masks[i] = gen_move_pattern(i, EMPTY,
				bishop_dirs, bishop_edges_a);
	}
}

static void
gen_rook_patterns(void)
{
	for (int i = 0; i < 64; ++i) {
...
transform_sliding_magics(void)
{
	...
	for (int i = 0; i < 64; ++i) {
		...
		if (i == 63) {
			...
			attack_count =
				(magics[(i+1)*4+2]>>8) - attack_offset_old;
		}
		...
		for (unsigned j = 0; j < attack_array_l; ++j) {
			attack_results[attack_offset_new+j] =
				attack_array[j] & ~bit64(i);
		}
