                         ...
                         player turn)
{
    ...
    *str++ = ' ';
    if (p.ep_ind == 0 || p.ep_ind == sq_h1) {
        *str++ = '-';
    }
    else {
        str = index_to_str(str, p.ep_ind-8, white);
    }
...
bool is_move_irreversible(const struct position *pos, move m)
{
    return (mtype(m) != general_move)
     ...
     || (mfrom(m) == sq_a1 && pos->castle_left_0)
     || (mfrom(m) == sq_h1 && pos->castle_right_0)
     || (mfrom(m) == sq_e1 && (pos->castle_left_0 || pos->castle_right_0))
     || (mto(m) == sq_a8 && pos->castle_left_1)
     || (mto(m) == sq_h8 && pos->castle_right_1);
}
...
{
    assert(empty(src->king_attack_map));
